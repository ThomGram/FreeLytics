...
class FreeworkSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        job_cards = response.css('div.mb-4.relative')  # Ajuste si besoin selon la structure réelle
    ...
    def parse_job_detail(self, response):
        ...
        try:
            ...
            company_description = response.css('div.flex.items-center.mr-6.mb-4 span::text').getall()
            # company_description = ' '.join(company_description).strip() if company_description else ''
            
            # Date de publication
            publication_date = response.xpath('/html/body/div/div/div/div/div[1]/div/div/div/div[1]/div[2]/div[1]/div[1]/span/text()').get() or ''
